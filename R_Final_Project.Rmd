...
```{r}
#This DESEQ ended up not being used in the final presentation, but I will leave the code here! 


rna_clinical$tumor_type <- ifelse(rna_clinical$definition == 'Solid Tissue Normal', 'normal', 'cancerous')

tumor_na_mask <- !is.na(rna_clinical$tumor_type)

rna_clinical <- rna_clinical[tumor_na_mask, ] 

#Nothing changed, still 175 columns as before
  
rna_counts <- rna_counts[,tumor_na_mask]
...
rna_clinical$tumor_type <- factor(rna_clinical$tumor_type, levels=c('normal', 'cancerous'))
head(rna_clinical$tumor_type)
...
results <- results(dds_obj, format = "DataFrame", contrast = c("tumor_type", 'normal', 'cancerous')) 
...
```{r}
#BEFORE ALL THIS SHIT, DO NOT RUN THE LOW COUNTS
...
```{r}
#Flipping rna_counts rows and columns
...
```{r}
#GET THE RESULTS FOR JUST CD38
results$gene <- rna_genes$gene_id
results_cd38 <- ifelse(results$gene == "ENSG00000004468.13", T, F)

results_cd38 <- results[results_cd38,]
...
ncol(countDataMatrix)
ncol(survival_clinic)

nrow(countDataMatrix)
...
```{r}
#Creating a lollipop to see if mutations are negligible on CD38!
```

```{r}
#Clinical data for GBM 
